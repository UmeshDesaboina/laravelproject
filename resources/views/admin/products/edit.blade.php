                ...
                <div class="bg-[#1f2937] rounded-xl border border-[#374151] p-6 shadow-xl">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="w-full px-4 py-3 bg-[#111827] border border-[#374151] rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#22c55e] focus:border-transparent transition-all" placeholder="Enter product name">
                            @error('name') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">SKU <span class="text-red-400">*</span></label>
                            <input type="text" name="sku" value="{{ old('sku', $product->sku) }}" required class="w-full px-4 py-3 bg-[#111827] border border-[#374151] rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#22c55e] focus:border-transparent transition-all" placeholder="e.g., PROD-001">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="5" class="w-full px-4 py-3 bg-[#111827] border border-[#374151] rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#22c55e] focus:border-transparent transition-all resize-none" placeholder="Detailed product description">{{ old('description', $product->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="price" value="{{ old('price', $product->price) }}" step="0.01" min="0" required class="w-full pl-8 pr-4 py-3 bg-[#111827] border border-[#374151] rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#22c55e] focus:border-transparent transition-all">
                            </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                                <input type="number" name="compare_price" value="{{ old('compare_price', $product->compare_price) }}" step="0.01" min="0" class="w-full pl-8 pr-4 py-3 bg-[#111827] border border-[#374151] rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#22c55e] focus:border-transparent transition-all" placeholder="Original price">
                            </div>
                        ...
                        <div class="grid grid-cols-4 gap-4 mb-6">
                            @foreach($product->images as $image)
                                <div class="relative group">
                                    <img src="{{ $image->url }}" alt="Product image" class="w-full aspect-square rounded-lg object-cover {{ $image->is_primary ? 'ring-2 ring-[#22c55e]' : '' }}">
                                    @if($image->is_primary)
                                    ...
                                    <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center space-x-2">
                                        @if(!$image->is_primary)
                                            <button type="button" onclick="setPrimaryImage({{ $image->id }})" class="p-2 bg-white/20 hover:bg-white/30 rounded-lg" title="Set as Primary">
                                                <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                                </svg>
                                            </button>
                                        @endif
                                        <button type="button" onclick="deleteImage({{ $image->id }})" class="p-2 bg-red-500/80 hover:bg-red-500 rounded-lg" title="Delete">
                                            <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-[#1f2937] rounded-xl border border-[#374151] p-6 shadow-xl">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <input type="number" name="quantity" value="{{ old('quantity', $product->quantity) }}" min="0" required class="w-full px-4 py-3 bg-[#111827] border border-[#374151] rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#22c55e] focus:border-transparent transition-all">
                        ...
                        <div>
                            ...
                            <input type="number" name="low_stock_threshold" value="{{ old('low_stock_threshold', $product->low_stock_threshold) }}" min="0" class="w-full px-4 py-3 bg-[#111827] border border-[#374151] rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#22c55e] focus:border-transparent transition-all">
                ...
                <div class="bg-[#1f2937] rounded-xl border border-[#374151] p-6 shadow-xl">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center">
                            <input type="radio" name="delivery_type" id="delivery_free" value="free" {{ old('delivery_type', $product->delivery_type ?? 'free') == 'free' ? 'checked' : '' }} class="w-4 h-4 text-[#22c55e] bg-[#111827] border-[#374151] focus:ring-[#22c55e] focus:ring-offset-0">
                            <label for="delivery_free" class="ml-3 text-gray-300">Free Delivery</label>
                        </div>
                        <div class="flex items-center">
                            <input type="radio" name="delivery_type" id="delivery_fixed" value="fixed" {{ old('delivery_type', $product->delivery_type ?? 'free') == 'fixed' ? 'checked' : '' }} class="w-4 h-4 text-[#22c55e] bg-[#111827] border-[#374151] focus:ring-[#22c55e] focus:ring-offset-0">
                            <label for="delivery_fixed" class="ml-3 text-gray-300">Fixed Charge</label>
                        </div>
                        <div id="delivery-charge-field" class="{{ (old('delivery_type', $product->delivery_type ?? 'free') == 'fixed') ? '' : 'hidden' }}">
                            <div class="relative">
                                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                                <input type="number" name="delivery_charge" value="{{ old('delivery_charge', $product->delivery_charge ?? 0) }}" step="0.01" min="0" class="w-full pl-8 pr-4 py-3 bg-[#111827] border border-[#374151] rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#22c55e] focus:border-transparent transition-all" placeholder="Delivery charge">
                            </div>
                        </div>
                    </div>
                </div>
